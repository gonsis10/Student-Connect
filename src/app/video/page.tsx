...
function App() {
  ...
  return (
    <div className="min-h-screen bg-white">
      <Navbar />
      <main className="container mx-auto px-2 pt-12 h-[calc(100vh-64px)]"> {/* Reduced padding and top margin */}
        <div className="flex gap-4 h-full"> {/* Reduced gap */}
          {/* Main Content */}
          <div className="flex-1 flex flex-col gap-4"> {/* Reduced gap */}
            {/* Video Grid */}
            <div className="grid grid-cols-2 gap-4 flex-1 h-[80vh]"> {/* Increased height */}
              {/* Local Video */}
          ...
          <div className="relative">
            <Button 
              ...
              className="absolute -left-12 top-4 p-2 bg-sky-400 hover:bg-sky-500 rounded-l-lg rounded-r-none"
            >
              <MessageCircle className="h-5 w-5" />
            </Button>

            <Card className={`w-80 h-full bg-white shadow-lg border-sky-100 transition-all duration-300 ${isQuestionPanelOpen ? "translate-x-0" : "translate-x-full"}`}> {/* Reduced width */}
              <div className="p-6 h-full flex flex-col">
                ...
                <Tabs value={activeTab} onValueChange={(value: string) => setActiveTab(value as QuestionCategory)} className="flex-1 flex flex-col">
                  <TabsList className="grid w-full grid-cols-3 mb-6 bg-sky-50">
                    <TabsTrigger value="funny" className="data-[state=active]:bg-sky-400 data-[state=active]:text-white">Fun</TabsTrigger>
                    <TabsTrigger value="getToKnow" className="data-[state=active]:bg-sky-400 data-[state=active]:text-white">Social</TabsTrigger>
                    <TabsTrigger value="deep" className="data-[state=active]:bg-sky-400 data-[state=active]:text-white">Deep</TabsTrigger>
                  </TabsList>
